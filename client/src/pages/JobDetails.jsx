...
const JobDetails = () => {
  ...
  useEffect(() => {
    const fetchJob = async () => {
      try {
        ...
        if (res.data.success) {
          setJobData(res.data.job); // ✅ correct setter
        } else {
  ...
  if (!jobData) return <p className="text-center mt-10 text-red-500">Job not found.</p>;
    ...
    <div className="container mx-auto my-10 p-6 md:p-12 bg-white rounded-2xl">
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-12 mb-8">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-600 text-lg">{jobData.companyId?.name}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="bg-purple-50 p-4 rounded-lg border-l-4 border-purple-500">
          <p className="text-gray-700"><strong>Location:</strong> {jobData.location}</p>
        </div>
        <div className="bg-green-50 p-4 rounded-lg border-l-4 border-green-500">
          <p className="text-gray-700"><strong>Internship Type:</strong> {jobData.internshipType}</p>
        </div>
        <div className="bg-blue-50 p-4 rounded-lg border-l-4 border-blue-500">
          <p className="text-gray-700"><strong>Category:</strong> {jobData.category}</p>
        </div>
